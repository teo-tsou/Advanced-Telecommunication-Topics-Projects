...
%   AEM: 2303
%   Project 2bii: m-PSK Flat Fading with Equalization system simulation - Deletion of c 
...
%       Equalization
        ...
        h_index=1;
        for j=1:length(y)
            y(j) = ((conj(h(h_index)+ c - mean(h+c)))/abs(h(h_index)^2)) * y(j);
            if mod(j,T*Rb)==0 
                h_index = h_index + 1;
            end
        end
...
figure(m+1);
title('BER Theoritical Equalization Diagram vs BER Simulation Equalization Diagram after the Deletion of c');
...
for i=1:length(M)
    ...
    title('BER Diagrams over SNR values after the Deletion of c');
    hold on;
end
