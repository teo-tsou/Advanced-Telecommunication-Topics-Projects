...
%     Tx Filtering
    ...
    for i = 1:length(SNRdb)
        ...
        real_noise = sqrt(noise_power) .* randn(1,length(sn)*4);
        imag_noise = sqrt(noise_power) .* randn(1,length(sn)*4);
        ...
        for j = 1:subcarriers:length(symbols)
          ...
          %Adding Noise
%            y = ofdm + w(j:j + subcarriers - 1 + cyclic_prefix) ; 
             yi = real(ofdm) + real_noise(j:j + subcarriers - 1 + cyclic_prefix);
             yq = imag(ofdm) + imag_noise(j:j + subcarriers - 1 + cyclic_prefix);
             y = yi + 1i*yq;
