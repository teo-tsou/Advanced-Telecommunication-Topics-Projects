...
%   AEM: 2303
%   Project 3a: 2x2 MIMO m-psk simulation with Least Squares Equalization
...
%     Tx Filtering
    ...
    for i = 1:length(SNRdb)
        ...
        real_noise1 = sqrt(noise_power) .* randn(length(new_sni/2),1);
        imag_noise1 = sqrt(noise_power) .* randn(length(new_snq/2),1);
        
        real_noise2 = sqrt(noise_power) .* randn(length(new_sni/2),1);
        imag_noise2 = sqrt(noise_power) .* randn(length(new_snq/2),1);
...
%       Multiplying different sn values (sn[i] , sn[i+1]) with different values of h11, h12, h21, h22 every T and Adding noise%
        ...
        k = 1;
        for j=2:2:length(sn)
            y1(k) = h11(h_index)*sn(j-1) + h21(h_index)*sn(j) +  w1(k);
            y2(k) = h12(h_index)*sn(j-1) + h22(h_index)*sn(j) +  w2(k);
            if mod(k,T)==0
                h_index = h_index + 1;
            end
            k = k + 1;
