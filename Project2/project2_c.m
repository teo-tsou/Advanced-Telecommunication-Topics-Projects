...
%   AEM: 2303
%   Project 2c: m-PSK BER simulation 
...
%         end
       ...
       %Flat Fading Channel - Two Different Channels%
        ...
        h1i = sqrt(1/sqrt(2))*randn(round(length(new_sni)/Rb)+1,1);
        h1q = sqrt(1/sqrt(2))*randn(round(length(new_sni)/Rb)+1,1);
        h1 = h1i + 1i*h1q ;

        h2i = sqrt(1/sqrt(2))*randn(round(length(new_sni)/Rb)+1,1);
        h2q = sqrt(1/sqrt(2))*randn(round(length(new_sni)/Rb)+1,1);
        h2 = h2i + 1i*h2q ;
